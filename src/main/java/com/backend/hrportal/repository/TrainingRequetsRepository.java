...
public interface TrainingRequetsRepository extends JpaRepository<TrainingRequest,Long> {
    ...
    @Query("select a from TrainingRequest a where a.user.id =:id")
    List<TrainingRequest> findMyTraining(@Param("id") Long id);

    @Query("select a from TrainingRequest a where a.training.id =:id")
    List<TrainingRequest> findByUser(@Param("id") Long id);
