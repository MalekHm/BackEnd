...
public class UserServiceI implements UserService {
    ...
    @Override
    public User uploadAvatar(UploadUserImageRequest uploadUserImageRequest) {
        ...
        try {
            Files.copy(uploadUserImageRequest.getFile().getInputStream(),
                    this.root.resolve(uploadUserImageRequest.getFile().getOriginalFilename()));
               Optional< User> user=userRepository.findById(uploadUserImageRequest.getId());
               user.get().setProfilImage(uploadUserImageRequest.getFile().getOriginalFilename());
               User user1=user.get();
            user1.setProfilImage(uploadUserImageRequest.getFile().getOriginalFilename());
    ...
    public User forgetPassword(String email) {
        ...
        if(user.isPresent()){
            ...
            user.get().setToken(String.valueOf(token));
            emailService.sendToken(email,String.valueOf(token));
